...
void keyredistribute(BDPTR T,int sib,BDPTR Tp){
    ...
    if(T->keys[0]>Tsib->keys[0]){//redistribute with left
...
void ins_bd(BDPTR &T,BDPTR Tp,int k){
    ...
    if(T->cnt==2*d+1){//overflow condition
        bool flag=1;
        if(Tp!=NULL){//current node has a parent
            int p=0;
            while(Tp->ptr[p]!=T)p++;
            int left=p-1,right=p+1,i=p;
            while(left>=0||right<=Tp->cnt){//finding first sibling node to the left or right where key redistribution is possible
                if(left>=0&&Tp->ptr[left]->cnt<2*d){
                    i=left;
                    flag=0;
                    break;
                }else if(right<=(Tp->cnt)&&Tp->ptr[right]->cnt<2*d){
                    i=right;
                    flag=0;
                    break;
                }
            ...
            while(i<p){
                keyredistribute(Tp->ptr[i+1],i,Tp);
                i++;
            }
            while(i>p){
                keyredistribute(Tp->ptr[i-1],i,Tp);
        ...
        if(flag)//case when all sibling nodes are full, then node split
            nodesplit(T,Tp);
    }
}
