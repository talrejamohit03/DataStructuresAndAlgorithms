...
int max_bst(AVLPTR T){
    if(T->rchild==NULL)
        return T->data;
    max_bst(T->rchild);
}
int min_bst(AVLPTR T){
    if(T->lchild==NULL)
        return T->data;
    min_bst(T->lchild);
}
void del_avlnode(AVLPTR &T,int k){
    if(T!=NULL){
        if(k<T->data)
            del_avlnode(T->lchild,k);
        else if(k>T->data)
            del_avlnode(T->rchild,k);
        else{
...
int main(){
    AVLPTR T1=NULL;
    int A[15]={7,6,5,4,3,2,1,15,14,13,12,11,10,9,8};
